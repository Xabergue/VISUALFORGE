...
async def create_task(body: dict, db: Session = Depends(get_db)):
    ...
    if not style or not subject:
        raise HTTPException(status_code=400, detail="Campos 'style' e 'subject' s�o obrigat�rios.")
    from routers.styles import STYLES
    valid_ids = [s["id"] for s in STYLES]
    if style not in valid_ids:
        raise HTTPException(status_code=400, detail=f"Estilo inv�lido: {style}")
...
async def get_task(task_id: str, db: Session = Depends(get_db)):
    ...
    if not task: raise HTTPException(status_code=404, detail="Task n�o encontrada.")
    return _task_to_dict(task)
...
async def stream_task(task_id: str, db: Session = Depends(get_db)):
    async def event_generator():
        ...
        while count < 3600:
            task = db.query(Task).filter(Task.id == task_id).first()
            if not task:
                yield f"data: {json.dumps({'error': 'Task n�o encontrada'})}\n\n"
                break
